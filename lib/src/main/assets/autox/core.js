...
function AutomationStep(name, javaAutomator) {
    ...
    const javaMethods = ["retry", "postActionDelay", "actionTargetIsFound", "preActionDelay"];
    ...
    const setupActionMethods = ["setupActionNode", "setupOptionalActionNode"];
    setupActionMethods.forEach(method => {
        this[method] = function(name, func) {
            return javaStep[method](name, {invoke: func});
        };
    });
...
function Automator(name) {
    ...
    const javaReadonlyProperties = ["allStepsSucceed", "message", "failedHierarchyString"];
    javaReadonlyProperties.forEach(prop => {
        Object.defineProperty(this, prop, {
...
var AppAutoContext = (function() {
    ...
    const javaReadonlyProperties = ["accessibilityServiceConnected", "notificationListenerConnected", "topAppHierarchyString", "autoSrv"];
    javaReadonlyProperties.forEach(prop => {
    ...
    const javaMethods = ["dumpTopActiveApp", "click", "setContent", "scrollUpDown"]
    javaMethods.forEach(method => {
        instance[method] = function() {
            return javaAppAutoContext[method].apply(javaAppAutoContext, arguments);
        }
    });
